...
<xsl:output method="xml" indent="yes" encoding="UTF-8" standalone="no"/>
	...
	<xsl:variable name="collectionName">
		<xsl:text>Leermiddelen_Educatheek</xsl:text>
	</xsl:variable>
...
<xsl:template match="/">
  <xsl:apply-templates select="//oai_dc:lom"/>
</xsl:template>

<xsl:template match="oai_dc:lom">
  <xsl:copy>
