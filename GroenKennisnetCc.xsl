...
<xsl:output method="xml" indent="no" encoding="UTF-8" standalone="no"/>
	...
	<xsl:variable name="collectionName">
		<xsl:text>GroenKennisnetCc</xsl:text>
	</xsl:variable>

	<!-- De door de collectie gebruikte namespace -->
	<xsl:variable name="usedNamespace">
		<xsl:text>oai_gkn</xsl:text>
	</xsl:variable>

<xsl:template match="/">
  <xsl:apply-templates select="//oai_gkn:lom"/>
</xsl:template>

<xsl:template match="oai_gkn:lom">
  <xsl:copy>
