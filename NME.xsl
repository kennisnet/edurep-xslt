...
<xsl:output method="xml" indent="no" encoding="UTF-8" standalone="no"/>
	...
	<xsl:variable name="usedNamespace">
		<xsl:text>nllom</xsl:text>
	</xsl:variable>

<xsl:template match="/">
  <xsl:apply-templates select="//nllom:lom"/>
</xsl:template>

<xsl:template match="nllom:lom">
  <xsl:copy>
