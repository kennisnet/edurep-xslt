<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="publisher">
    <xsl:choose>
      ...
      <xsl:otherwise>Natuurtijdschriften repository</xsl:otherwise>
    </xsl:choose>
  </xsl:variable>

  <xsl:variable name="bron">
    <xsl:choose>
      <xsl:when test="//dc:source">
        <xsl:value-of select="//dc:source/text()"/>
      </xsl:when>
      <xsl:otherwise>geen bron</xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  ...
  <xsl:template match="/">
    <xsl:element name="czp:lom" namespace="http://www.imsglobal.org/xsd/imsmd_v1p2">
      ...
      <xsl:element name="czp:lifecycle">
        <xsl:call-template name="IMSvocabulary">
          <xsl:with-param name="element" select="'czp:status'"/>
          ...
          <xsl:for-each select="//dc:creator">
            <xsl:element name="czp:contribute">
              ...
              <xsl:call-template name="czp-contributecentity">
                ...
                <xsl:with-param name="vcard_n" select="."/>
              </xsl:call-template>
            </xsl:element>
          </xsl:for-each>
        </xsl:if>
        ...
        <xsl:if test="count(//dc:publisher) = 0">
          <xsl:element name="czp:contribute">
            <xsl:call-template name="IMSvocabulary">
              <xsl:with-param name="element" select="'czp:role'"/>
              <xsl:with-param name="source" select="$vdex_contributerole"/>
              <xsl:with-param name="value" select="'content provider'"/>
            </xsl:call-template>
            <xsl:call-template name="czp-contributecentity">
      ...
      <xsl:if test="$bron">
        <xsl:element name="czp:relation">
          ...
          <xsl:element name="czp:resource">
            <xsl:call-template name="IMScatalogentry">
              <xsl:with-param name="catalog" select="'uri'"/>
              <xsl:with-param name="entry" select="'www.natuurtijdschriften.nl'"/>
            </xsl:call-template>
            <!-- Add description -->
            <xsl:call-template name="IMSlangstring">
              <xsl:with-param name="element" select="'czp:description'"/>
              <xsl:with-param name="language" select="'x-none'"/>
              <xsl:with-param name="value" select="$bron"/>
            </xsl:call-template>
          </xsl:element>
        </xsl:element>
      </xsl:if>
      ...
      <xsl:if test="//dc:type">
          <xsl:call-template name="IMSclassification">
            ...
            <xsl:with-param name="taxon_source" select="'https://library.wur.nl/vdex/info-eu-repo.rdf'"/>
